...
Версия | Описание
--- | ---
1.5 | - При навешивании загрузчика на кнопку в `onComplete` теперь также передаётся параметр `btn`<br>- Появилась функция `onChange`, вызываемая сразу после выбора файла, она может запустить загрузку файла на сервер или отменить её
...
var onChange = function(callback) {
    // ...
    // здесь какой-то код
    // может даже с ajax запросами
    // который может сделать callback(true);
    // или callback(false);
    // ...
};
lemurro.file.bindUpload(btn, onComplete, onChange);
```
